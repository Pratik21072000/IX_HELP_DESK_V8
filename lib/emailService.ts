      ...
      <div class="container">
        ...
        <div class="content">
          <div class="ticket-info">
            ...
            <div class="info-row">
              <span class="label">Department(s):</span>
              <span class="value">${
                departmentNames ||
                (() => {
                  try {
                    const depts = JSON.parse(ticket.department);
                    return depts
                      .map(
                        (dept: string) =>
                          DEPARTMENT_LABELS[
                            dept as keyof typeof DEPARTMENT_LABELS
                          ],
                      )
                      .join(", ");
                  } catch {
        ...
        <div class="footer">
          <p>HelpDesk System | Multi-Department Ticket</p>
          <p style="font-size: 12px; margin: 5px 0;">
            This is an automated notification. Please do not reply to this email.
          </p>
        </div>
...
---
HelpDesk System | Multi-Department Ticket
This is an automated notification.
  `.trim();
}

/**
 * Send email notification for new ticket to multiple departments
 */
export async function sendTicketCreatedEmail(
  ticket: Ticket & { user: User },
): Promise<void> {
  try {
    ...
    try {
      departments = JSON.parse(ticket.department);
    } catch (error) {
      console.error("Failed to parse departments JSON:", error);
      return;
    }

    // Get email addresses for all departments
    const departmentEmails = departments
      .map((dept) => DEPARTMENT_EMAILS[dept as keyof typeof DEPARTMENT_EMAILS])
      .filter((email) => email); // Remove undefined emails

    if (departmentEmails.length === 0) {
      console.error(
        `No email configured for departments: ${departments.join(", ")}`,
      );
      return;
    }
    ...
    const departmentNames = departments
      .map((dept) => DEPARTMENT_LABELS[dept as keyof typeof DEPARTMENT_LABELS])
      .join(" & ");
    ...
    const emailParams = {
      ...
      Message: {
        ...
        Body: {
          Html: {
            Data: generateTicketEmailHTML(ticket, departmentNames),
            Charset: "UTF-8",
          },
          Text: {
            Data: generateTicketEmailText(ticket, departmentNames),
            Charset: "UTF-8",
          },
        },
      },
    ...
    console.log(
      `Email sent successfully for ticket #${ticket.id} to departments: ${departments.join(", ")}`,
      response,
    );
  } catch (error) {
    console.error("Failed to send ticket notification email:", error);
